...
pageScanner.prototype = {
	files: {
		links: 'links.txt',
		scanned_links: 'scanned_links.txt'
	},
	path: '#!/',
	loaded: false,
	...
	initialize: function(args){
		this.fs = require('fs');
                ...
                this.baseUrl = args['base'];
                
                //this.fs.changeWorkingDirectory('..');
	},
	
	begin: function(callback){
		this.page = new WebPage();
		this.page.loadImages = false;
				
		console.log("Load Url: " + this.sourceUrl + this.path);
		var _this = this;
		
		this.page.open(this.sourceUrl + this.path, function(status){
		    if (status !== 'success') {
		        console.log('Unable to load the address: ' + this.path);
		        return false;
		    } else {
	...
	scanLinks: function(){
		var links = [], fileStream;
	    
	    console.log('Scanning Links');
	    
	    links = this.page.evaluate(function() {
        ...
        fileStream = this.fs.open(this.files.links, 'a');		
		
		_.each(links, function(link, index){
				var matches = link.match(/#!\/(.+)/);
				if(matches){
					fileStream.writeLine(matches[0]);
				}
		});
	...
	markScanned: function(){
		var fileStream;
		
		fileStream = this.fs.open(this.files.scanned_links, 'a');
		fileStream.writeLine(this.path);
		fileStream.close();
		this.removeDuplicates();
	},
	
	removeDuplicates: function(){
		var scanned_links = [], unscanned_links = [], fileStream;
	    
	    fileStream = this.fs.open(this.files.links, 'r');
	    unscanned_links = fileStream.read().split("\n");
	   	fileStream.close();
	    
	    fileStream = this.fs.open(this.files.scanned_links, 'r');
	    scanned_links = fileStream.read().split("\n");
		fileStream.close();	    
	    
	    unscanned_links = _.uniq(unscanned_links);
	    unscanned_links.sort();	    
	    
	    scanned_links = _.uniq(scanned_links);
	    scanned_links.sort();

		unscanned_links = _.difference(unscanned_links, scanned_links);
	    
	    fileStream = this.fs.open(this.files.scanned_links, 'w');
	    fileStream.write(scanned_links.join("\n") + "\n");
		fileStream.close();	   
		
	    fileStream = this.fs.open(this.files.links, 'w');
	    fileStream.write(unscanned_links.join("\n") + "\n");
		fileStream.close();		
	},
	...
	writeFile: function(){
		var fileStream, path, baseTag, hashScript;
                baseTag = '<base href="'+this.baseUrl+'"/>';
                hashScript = '<script type="text/javascript">window.location.hash="'+this.path+'";</script>'
		...
		path += 'index.html';
                ...
                fileStream = this.fs.open(path, 'w');
                
                fileStream.write(this.page.content
                                    .replace(/<head([^>]+)?>/i, '<head$1>'+baseTag+hashScript)
                                    .replace(/<script[^>]+(async|data-requirecontext)[^>]+><\/script>/gi, ''));
	...
	hasLinks: function(){
		var fileStream, links;
		
	    fileStream = this.fs.open(this.files.links, 'r');
	    links = fileStream.read().split("\n");
	   	fileStream.close();
	...
	exit: function(){
		this.fs.remove(this.files.scanned_links);
                this.fs.remove(this.files.links);
...
phantom.args.forEach(function (arg, i) {
    arg = arg.split('=');
    window.arguments[arg[0]] = arg[1];
});

var arguments = window.arguments;

if(typeof arguments == 'undefined' || typeof arguments.source == 'undefined' || typeof arguments.base == 'undefined'){
    console.log('Please define a valid source and base URL.');
    phantom.exit();
}

var scanner = new pageScanner();

scanner.initialize(arguments);

function processPage(){
	scanner.begin(function(){
		scanner.scanLinks();
		scanner.writeFile();
		
		if(scanner.hasLinks() && typeof scanner.path != 'undefined' && scanner.path){
			processPage();
		} else {
